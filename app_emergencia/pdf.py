...
def dibujarParteZonaCuerpo(
    ...
    if parte:
        lista = filter(
            lambda x: x.partecuerpoR() == busqueda,
            listExamen)
    else:
        lista = filter(
            lambda x: x.zonacuerpoR() == busqueda,
            listExamen)
...
def historia_med_pdf(request, id_emergencia):
    ...
    atList = Atencion.objects.filter(
        emergencia=id_emergencia)  # Lista Atencion
    ...
    c.drawString(
        ...
        "Fecha: "
        +
        str(emer.hora_ingresoReal.strftime("%d/%m/%y"))
    )
    c.drawString(
        3*inch,
        8.9*inch,
        "Hora de Llegada: "
        +
        str(emer.hora_ingresoReal.strftime("%H:%M"))
    )
    ...
    c.drawString(
        ...
        "Nombre del Médico:   "
        +
        emer.responsable.first_name
        +
        " "
        +
        emer.responsable.last_name
    )
    ...
    # medicos
    antMed = Pertenencia.objects.filter(
        paciente=emer.paciente,
        antecedente__tipo="medica"
    )  # busco info en BD
    i = restar(c, i, 0.2)
    c.setFont("Helvetica", 11)
    ...
    # quirurgicos
    antQui = Pertenencia.objects.filter(
        paciente=emer.paciente,
        antecedente__tipo="quirurgica"
    )  # busco info en BD
    i = restar(c, i, 0.3)
    c.setFont("Helvetica", 11)
    ...
    # alergicos
    antAler = Pertenencia.objects.filter(
        paciente=emer.paciente,
        antecedente__tipo="alergia"
    )  # busco info en BD
    i = restar(c, i, 0.3)
    c.setFont("Helvetica", 11)
    ...
    # busco info en BD
    examen = AspectoAtencion.objects.filter(
        atencion=aten,
        revisado="1")  # Lista Atencion
    examen = list(examen)
    examen = filter(
        lambda x: x.estadoR() == "anormal",
        examen
    )	 # filtro para obtener solo las partes alteradas

    i = dibujarParteZonaCuerpo(
        c,
        i,
        True,
        examen,
